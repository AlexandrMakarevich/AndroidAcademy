...
class MovieDBRepositoryDetails {

    @ExperimentalSerializationApi
    suspend fun loadMovie(id: Int) : Movie = withContext(Dispatchers.IO) {

        val movieDetailsRetrofit = RetrofitModule.movieDBApi.getMovieDetails(id)
        val movieCast: ListActors = RetrofitModule.movieDBApi.getActors(id)

        return@withContext parseToMovie(movieDetailsRetrofit, movieCast)
    }

    private fun parseToMovie(movieDetailsRetrofit: MovieDetailsRetrofit, movieCast: ListActors) : Movie {
        return Movie(
            ...
            actors = movieCast.cast.map { Actor(it.id, it.name, "$base_url$profile_sizes${it.picture}") }
        )
    }
    companion object {
